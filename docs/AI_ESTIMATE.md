...
> ステータス: 旧企画メモ。現行仕様は `docs/reference/README_AI_ESTIMATE.md` を優先。

> 注: この文書は「要件整理チャット前提」の旧案です。現在は Google Drive 上の要件定義書を起点に AI ドラフト生成する方式へ移行済みです。
...
- チャットの想定閲覧者: 内部用のみ。口調は社内向けでOK。
- 見積AIへの連携タイミング: 現状と同じフロー（要件整理の最新版を使ってAIドラフト生成→プレビュー→採用/置換）。

## 6. 今回依頼分のUI/仕様整理（実装はまだしない）
- 要件整理チャット/手動入力の二択を残しつつ、AI整理結果プレビューの下に「要件概要を生成」ボタンを追加。
  - プレビューに表示された（直近のAI応答）から「要件概要」を生成して表示する。
  - 機能要件・非機能要件・未確定要件の3つを常に1セット保持し、生成のたびに上書き（手動編集も上書き前提）。
- 「AIでドラフト見積生成」ボタンをプレビューの下に設置し、既存のドラフト生成機能と同一の挙動にする。
  - 重複ボタンは廃止し、チャット/手動入力どちらのモードでも同じ起点を使う。
- 目的: 要件整理→要件概要への反映→そのままドラフト見積生成、の導線を1か所にまとめてブレをなくす。

...
- 社内ビューでは「要件定義書から AIドラフト生成」カードを提供し、Google Drive から選択した要件定義書 URL (`google_docs_url`) を添付要件と AI 解析元で兼用する。
- AI は要件定義書から `requirement_summary` / 機能要件 / 非機能要件 / 未確定要件 / 備考生成プロンプト候補を抽出し、そのままドラフト明細と備考案を生成する（`POST /estimates/ai/generate-draft-from-doc`）。
- 「備考生成プロンプト」は未保存見積でも現在フォームの文脈（顧客、案件名、明細、要件要約、要件定義書 URL）を backend に渡して OpenAI 経由で備考（対外）を生成する（`POST /estimates/generate-notes`）。
...
- 設計/開発明細が含まれる場合、`google_docs_url` は必須。Google Drive Picker 未設定環境では URL 手入力も可能だが、AIドラフト生成には Drive 認証済みの文書選択が必要。

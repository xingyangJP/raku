...
### Step 29: `.claude/` を ignore 対象へ追加
- 空の未追跡ディレクトリ `.claude/` を Git 管理対象から外す依頼を受領。
- 既存 `.gitignore` を確認し、`.claude/` の除外設定が未登録であることを確認。
- 最小差分で `.gitignore` に `.claude/` を追加する方針を確定。

### Step 30: `main` マージ準備
- `main` へのマージ依頼を受領し、現在の `dev` / `main` の差分と未コミット差分を確認。
...
### Step 32: 計画工数グラフ未反映の原因切り分け
- ダッシュボード画面のスクリーンショットを確認し、凡例・最新値には計画工数が出ている一方で線が底に張り付いて見えることを確認。
- `Dashboard.jsx` の `SimpleComboChart` を調査し、金額系列と工数系列が単一スケールで描画されていることを確認。
- 計画工数データ未反映ではなく、金額と人日の桁差による表示スケール不整合が原因と判断。

### Step 33: グラフ修正の `dev` 反映準備
- 依頼内容を「今回のグラフ修正を push し、`dev` に反映する」と整理。
...
- Step 150: 更新履歴とヘルプが同画面なのは責務混在なので、更新履歴を専用ページ `/release-notes` に分離。モーダルとサイドメニューの遷移先を専用ページへ変更し、ヘルプ画面から履歴一覧は撤去した。
- Step 151: 左メニュー下部のバージョン表記を撤去し、左上の `KCS販売管理` タイトル右へ移動。表示内容は維持しつつ、重複感のある下部表示を整理した。
- Step 152: 更新履歴ページ先頭のバージョン表示だけ `ver. v1.0.19` という別書式だったため、最新履歴カードと同じ `v1.0.19` 表記へ統一。表示基準も `releaseNotes.latest.version` に一本化した。

## 2026-04-03

- Step 153: 検収書発行機能の要件整理。要望を整理し、「注文書を発行した時点で検収書を出せる」導線を追加する方針を確定。既存導線を調査し、主導線を見積編集画面、補助導線を受注一覧に置く構成を採用。
